...
\begin{itemize}
\item \textbf{PUF}: Physically Unclonable Function (PUF) is a security metric in hardware sistems leveraging inherent device physical variations to produce an unclonable, 
unique device response to a given input implementing a challenge-response type of authentication. Unclonability means that each PUF has a unpredictable response to a given input, because the response is created by complex interactions between many random components.
They act as a unique device identifier.

\item \textbf{Challenge-Response Authentication}: authentication serves as a robust method based on a unique challenge provided to a device by an authenticator.
...
\item \textbf{Fixed Pattern Noise}: it is a particular noise pattern observed in digital imaging sensors due to small differences in specific sensor pixels, leading to variations in pixel brightness or darkness. 
    This phenomenon is created from two sources: PRNU (Photo Response Non-Uniformity) and DSNU (Dark Signal Non-Uniformity). The latter will be the focus of our PUF implementation.

    \item \textbf{PRNU}: Photo response non-uniformity, pixel response non-uniformity, or PRNU, is a form of fixed-pattern noise related to digital image sensors, as used in cameras and optical instruments. It happened due to the non-uniformity of image sensors, each cell responds with a different voltage level when illuminated with a uniform light source, and this leads to luminance inaccuracy at the pixel level.

\item \textbf{DSNU}: Dark Signal Non-Uniformity is a form of noise encountered in dark images. The intensity values for pixels does not start from zero, an offset is added on every pixel,
...
\textbf{Secure Key Generation}: PUF responses can be utilized to generate cryptographic keys without the need for additional storage of secret keys, enhancing the security of cryptographic protocols.This applciation is divided in Secret Key Distribution and Secret Key Storage.
